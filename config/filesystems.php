...
// Whether object storage (S3-compatible) credentials are actually available —
// NOT the same thing as FILESYSTEM_DISK's value. On Laravel Cloud that env var
// holds the *name* of the provisioned disk (e.g. "private"), not a Flysystem
// driver, so it can never be used directly as one.
$usingObjectStorage = $laravelCloudDisk !== null || env('AWS_ACCESS_KEY_ID');
...
return [
    ...
    */
    ...
    'disks' => [
        ...
        // name ("public"), not the framework default. Local dev stays on the "local"
        // driver (disk storage); anywhere object storage credentials are present
        // (Laravel Cloud's disk config, or plain AWS_* vars) it switches to "s3" so
        // uploads land in real object storage instead of a container-local disk that
        ...
        'public' => [
            ...
            'url' => $laravelCloudDisk['url']
                ?? env('AWS_URL')
                ?? ($usingObjectStorage ? null : rtrim(env('APP_URL', 'http://localhost'), '/').'/storage'),
            'visibility' => 'public',
            'throw' => false,
            'report' => false,
